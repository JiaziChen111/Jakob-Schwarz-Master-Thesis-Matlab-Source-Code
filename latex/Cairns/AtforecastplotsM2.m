...
arima011=arima(0,1,1);
 ...
 ErrM=zeros(8,2);
 for i=1:6
 [ ~,~,MeanErr,Err2,~,~]=errorfkt_q( q_true(31:end,1:end),q_C_future{i}(31:end,1:19) );
 ErrM(i,1)=Err2;
 end
 [ ~,~,MeanErr,Err2,~,~]=errorfkt_q( q_true(31:end,1:end), q_wLC_011(70+1:x_max_C+1,1:19) );
 ErrM(7,1)=Err2;
 [ ~,~,MeanErr,Err2,~,~]=errorfkt_q( q_true(31:end,1:end), q_wLC_211(70+1:x_max_C+1,1:19) );
 ErrM(8,1)=Err2;
...
print(errq,'errqfutureb','-dpdf','-r0')
...
pos = get(errq,'Position');
set(errq,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
print(errq,'errqfuturewLCb','-dpdf','-r0')
...
pos = get(errq,'Position');
set(errq,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
print(errq,'errqfuture70b','-dpdf','-r0')
%}
