...
hold on;
plot(linspace(1,x,x),a_ML_newton2(1:x),'m');
hold on;
plot(linspace(1,x,x),a_ML(1:x),'b');
hold on;
%plot(linspace(1,x,x),a_ML2(1:x),'m');
title('a_x');

subplot(2,2,2);
plot(linspace(1,x,x),b_ML_newton(1:x),'c');
hold on;
plot(linspace(1,x,x),b_ML_newton2(1:x),'m');
hold on;

plot(linspace(1,x,x),b_ML(1:x),'b');
hold on;
%plot(linspace(1,x,x),b_ML2(1:x),'m');
title('b_x');
...
hold on;
plot(linspace(1,t,t),k_ML_newton2(1:t),'m');
hold on;

plot(linspace(1,t,t),k_ML(1:t),'b');
hold on;
%plot(linspace(1,t,t),k_ML2(1:t),'m');

title('k_t');
legend({'ML_{newton}','ML_{newton3step}','ML_{normalen}'},'location','best');
set(hhh,'Units','Inches');
pos = get(hhh,'Position');
set(hhh,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
print(hhh,'gdpabkML','-dpdf','-r0')

hhhhh=figure;

plot(linspace(1,x,x),y_ML_newton(1:x),'c');
hold on;
plot(linspace(1,x,x),y_ML_newton2(1:x),'m');
hold on;

plot(linspace(1,x,x),y_ML(1:x),'b');
hold on;
%plot(linspace(1,x,x),y_ML2(1:x),'m');
