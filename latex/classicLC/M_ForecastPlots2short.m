...
hold on
plot([t_max,time_future],[M_wLC2(age+1,end),M_wLC2_Forecast(age+1,:)],'r--')
%plot(time_future,M_wLC_Forecast_UB(age+1,:),'r:')
%plot(time_future,M_wLC_Forecast_LB(age+1,:),'r:')
hold on
plot([t_max,time_future],[M_D2(age+1,end),M_D2_Forecast(age+1,:)],'k--')
%plot(time_future,M_D_Forecast_UB(age+1,:),'k:')
%plot(time_future,M_D_Forecast_LB(age+1,:),'k:')
hold on
plot([t_max,time_future],[M_ML2(age+1,end),M_ML2_Forecast(age+1,:)],'b--')
...
ax1 = subplot(3,1,3);
%plots development over time for a specific age
age=100; %1:x enstpricht x_min:x_max %%%%%%%%%%%%%%%%%%% choose
...
print(forecast1,'forecast1short','-dpdf','-r0')
...
plot(ages,lnM_wLC2_Forecast(:,year),'r')
hold on
plot(ages,lnM_D2_Forecast(:,year),'k')
hold on
plot(ages,lnM_ML2_Forecast(:,year),'b')
%hold on
...
%hold on
%plot(ages,lnM_W_Forecast(:,year),'c')
legend({'historical','wLC2','D2','ML2'},'Location','northwest')
...
print(forecast2,'forecast2short','-dpdf','-r0')

%%%%%%%%%%
errforecast=figure;
subplot(2,2,1)
tempM=M_full(1:x,t+1:end);
index42=tempM==42;
Merr= tempM- M_ML2_Forecast(:,1:t_full-t);
Merr(index42)=0;
xscale = [1997 2015];
yscale = [0 100];
...
xlabel('year t')
ylabel('age x')
suptitle('ML2 forecast error colormap')
...
print(errforecast,'errforecastMLshort','-dpdf','-r0')

errforecast=figure;
subplot(2,2,1)
tempM=M_full(1:x,t+1:end);
index42=tempM==42;
Merr= tempM- M_wLC2_Forecast(:,1:t_full-t);
Merr(index42)=0;
xscale = [1997 2015];
yscale = [0 100];
imagesc(xscale,yscale,Merr)
...
xlabel('year t')
ylabel('age x')
suptitle('wLC2 forecast error colormap')

set(errforecast,'Units','Inches');
pos = get(errforecast,'Position');
set(errforecast,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
print(errforecast,'errforecastwLCshort','-dpdf','-r0')
...
xlabel('year t')
ylabel('age x')
suptitle('D2 forecast error colormap ')

set(errforecast,'Units','Inches');
pos = get(errforecast,'Position');
set(errforecast,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
print(errforecast,'errforecastDshort','-dpdf','-r0')
