...
%1956-1996 (1:46)
g=[g_full(7:46)';rauch(7:46)';alk(7:46)];
...
% legend('West Germany','Germany','Forecast')
...
xscale = [1997 2015];
yscale = [0 100];
imagesc(xscale,yscale,Merr{6,2})
colorbar
caxis([-1 1]);
xlabel('year t')
ylabel('age x')
...
hold on;
plot(linspace(1,x_cut-1 ,x_cut ),a_x{6}(1:x_cut ),'b');
hold on;
...
hold on;
plot(linspace(1,x_cut-1 ,x_cut ),b_x{6}(1:x_cut ),'b');
hold on;
plot(linspace(1,x_cut-1 ,x_cut ),b_renormN(1:x_cut ),'m');
hold on;
plot(linspace(1,x_cut-1 ,x_cut ),b_xN(1:x_cut ),'c');
legend({'wLC-renormed','ML-newton','wLC-renormed_{alt}','ML-newton_{alt}'},'location','best');
...
hold on;
plot(linspace(t+1,t+t_future ,t_future),k_future{6,2},'b--');
hold on;
...
hold on;
plot(linspace(1,x_cut-1 ,x_cut ),y_x{6}(1:x_cut ),'b');
hold on;
